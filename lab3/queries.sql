...
select Н_ЛЮДИ.ИМЯ, count(Н_ЛЮДИ.ИМЯ) as name_count, Н_ЛЮДИ.ФАМИЛИЯ, count(Н_ЛЮДИ.ФАМИЛИЯ) as surname_count
from Н_УЧЕНИКИ
...
select Н_ЛЮДИ.ИМЯ, count(*) as count
from Н_ЛЮДИ
...
-- Вывести самого молодого принимающего экзамен на данный момент
select ФАМИЛИЯ,
       ИМЯ,
       ОТЧЕСТВО,
       ДАТА_РОЖДЕНИЯ,
       ...
       УЧГОД
from Н_ЛЮДИ as Л
         join Н_СЕССИЯ on Н_СЕССИЯ.ЧЛВК_ИД = Л.ИД
where Л.ДАТА_СМЕРТИ = '9999-09-09 00:00:00.000000'
--   and Н_СЕССИЯ.ДАТА is not null
--   and Н_СЕССИЯ.АУДИТОРИЯ is not null
order by Л.ДАТА_РОЖДЕНИЯ desc
limit 1;